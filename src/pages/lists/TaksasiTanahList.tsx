...
export default function TaksasiTanahList() {
  const { user } = useAuth();
  ...
  // Filter only Tanah type
  const tanahList = useMemo(() => {
    let filtered = taksasiList.filter(t => t.jenis_agunan === 'Tanah');
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Taksasi Tanah"
        description="Daftar penilaian agunan berupa tanah"
        actions={
          !isDemo && (
            <Button onClick={() => navigate('/taksasi/tanah/new')}>
            ...
            {tanahList.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                  {searchTerm ? 'Tidak ada hasil yang ditemukan' : 'Belum ada data taksasi tanah'}
                </TableCell>
              </TableRow>
            ) : (
              tanahList.map((taksasi, index) => (
                    ...
                    <div className="flex items-center justify-center gap-1">
                      ...
                      {!isDemo && (
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => navigate(`/taksasi/tanah/edit/${taksasi.id}`)}
                          title="Edit"
                        >
                          <Pencil size={16} />
                        </Button>
      ...
      {tanahList.length > 0 && (
        <div className="text-sm text-muted-foreground">
          Menampilkan {tanahList.length} data taksasi tanah
        </div>
      )}
    </div>
  );
